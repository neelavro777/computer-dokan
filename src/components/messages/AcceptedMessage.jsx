          ...
          <div className={`d-inline-block rounded-pill px-3 py-2 ${messageClass} ${bubbleClass}`}>
            ...
            {!fromMe ? (
                authUser.userType === 'seller' ? (
                    ...
                    <div>
                        <h1> You have accepted this seller's offer </h1>
                    </div>
                )
            ) : (
                authUser.userType === 'seller' ? (
                    <div>
                        <h1> Buyer has accepted your offer </h1>
                    </div>
                ) : (
                    <div>
            ...
            {authUser.userType === 'customer' && (
              <button className="btn btn-dark" onClick={handleAddToCart} disabled={isAddedToCart === chat.offer.product._id}>Add to Cart</button>
            )}
          </div>
        </div>
      </div>
