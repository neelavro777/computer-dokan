...
const ImageUpload = ({ onUpload }) => {
  ...
  const onUploadItem = async () => {
    ...
    const specificationsArray = forms.map(form => {
      return { [form.name]: form.inputs.map(input => ({ [input.key]: input.value })) };
    });
    formData.append('specifications', JSON.stringify(specificationsArray));

    try {
      const res = await axios.post("http://localhost:5000/api/product/upload-item", formData, {
        headers: {
    ...
    <div>
      <form onSubmit={onUploadItem}>
        <div className="w-50">
          ...
          <input className="form-control" type="text" placeholder="Product" aria-label="Product" value={product} onChange={(e) => setProduct(e.target.value)}/>
          <input className="form-control" type="text" placeholder="Category" aria-label="Category" value={category} onChange={(e) => setCategory(e.target.value)}/>
          <input className="form-control" type="text" placeholder="Price" aria-label="Price" value={price} onChange={(e) => setPrice(e.target.value)}/>
              ...
              {form.inputs.map((input, inputIndex) => (
                <div key={inputIndex}>
                  <input
                    ...
                    placeholder="Key"
                    value={input.key}
                    onChange={(e) => handleInputChange(formIndex, inputIndex, 'key', e.target.value)}
                  />
                  <input
                    type="text"
                    placeholder="Value"
                    value={input.value}
                    onChange={(e) => handleInputChange(formIndex, inputIndex, 'value', e.target.value)}
                  />
                </div>
              ))}
              <button type="button" onClick={() => handleAddInput(formIndex)}>Add Input</button>
            </div>
          ))}
